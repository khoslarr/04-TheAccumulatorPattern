...
Authors: David Mutchler, Vibha Alangar, Matt Boutell, Dave Fisher, Mark Hays,
         Aaron Wilkin, their colleagues, and Rishav Khosla.
...
def run_test_draw_circles_from_rectangle():
    ...
    rect1.outline_color = 'black'
    draw_circles_from_rectangle(4, 5, rect1, window)

    # Test 2:
    rect2 = rg.Rectangle(rg.Point(500, 450), rg.Point(600, 400))
    rect2.fill_color = 'blue'
    rect2.outline_thickness = 3
    rect2.outline_color = 'red'
    draw_circles_from_rectangle(8, 3, rect2, window)

    window.close_on_mouse_click()

    # Test 3:
    window2 = rg.RoseWindow(620, 380)
    rect3 = rg.Rectangle(rg.Point(350, 280), rg.Point(375, 330))
    rect3.fill_color = 'yellow'
    rect3.outline_thickness = 5
    rect3.outline_color = 'brown'
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    x1 = rectangle.get_upper_left_corner().x - rectangle.get_height()/2
    y1 = (rectangle.get_upper_left_corner().y + rectangle.get_lower_left_corner().y)/2
    for _ in range(m):
        circle = rg.Circle(rg.Point(x1, y1), rectangle.get_height()/2)
        circle.fill_color = rectangle.fill_color
        circle.attach_to(window)
        x1 = x1 - circle.radius * 2
    x2 = (rectangle.get_upper_left_corner().x + rectangle.get_upper_right_corner().x)/2
    y2 = rectangle.get_upper_right_corner().y - rectangle.get_width()/2
    for _ in range(n):
        circle = rg.Circle(rg.Point(x2, y2), rectangle.get_width()/2)
        circle.outline_color = rectangle.outline_color
        circle.attach_to(window)
        y2 = y2 - circle.radius * 2
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    for k in range(n):
        ...
        start.x = start.x - rectangle1.get_width()/2
        start.y = start.y + rectangle1.get_height()/2
        end.x = end.x - rectangle1.get_width()/2
        end.y = end.y + rectangle1.get_height()/2
